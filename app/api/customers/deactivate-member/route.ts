...
export async function POST(req: NextRequest) {
  try {
    ...
    // Cannot deactivate the account owner (team_leader or solo_agent)
    if (user.role === 'team_leader' || user.role === 'solo_agent') {
      return NextResponse.json(
        { error: 'Cannot deactivate the account owner. Delete the customer account instead.' },
        { status: 400 }
      )
    }
    ...
    await logAdminAction(admin.adminId, 'deactivate_team_member', {
      targetTeamId: teamId,
      targetUserId: userId,
      reason: reason.trim(),
